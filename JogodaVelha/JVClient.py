...
class Jogador:
    def __init__(self):
        ...
        self.revanche_solicitacao_evento = threading.Event()
        self.revanche_resposta_evento = threading.Event()
    ...
    def finalizar(self):
        ...
        self.revanche_solicitacao_evento.set()
    ...
    def revanche(self):
        self.revanche_resposta_evento.clear()
        self.revanche_solicitacao_evento.set()
        self.revanche_resposta_evento.wait()
...
def main():
    ...
    try:
        while jogador.jogo_ativo:
            # Espera até que o servidor solicite uma JOGADA ou uma REVANCHE
            while jogador.jogo_ativo:
                
                if jogador.revanche_solicitacao_evento.is_set():
                    jogador.revanche_solicitacao_evento.clear()

                    while True:
                        resp = input("\nDeseja jogar uma revanche? (s/n): ").strip().lower()
                        if resp in ['s', 'n']:
                            jogador.resposta_revanche = (resp == 's')
                            break
                        print("Opção inválida. Digite 's' para sim ou 'n' para não.")

                    
                    jogador.revanche_resposta_evento.set()
                    break

                elif jogador.turno_evento.is_set():
